...
function Lightbox({ images, initialIndex, onClose }: LightboxProps) {
  ...
  return (
    ...
    >
      {/* Close */}
      <button
        className="absolute top-4 right-4 flex items-center gap-2 px-4 py-2 rounded-full bg-white/15 hover:bg-white/25 text-white text-sm font-medium transition-colors border border-white/20"
        onClick={onClose}
        aria-label="Close"
      >
        <X className="w-4 h-4" />
            ...
            <SheetHeader>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <SheetTitle className="text-xl mb-1 leading-snug">{project.title}</SheetTitle>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {detail.impact.map((item, i) => (
                  <div
                    key={i}
                    className="p-3 border border-border/60 rounded-xl bg-card hover:border-orange-500/40 transition-colors text-sm text-muted-foreground"
                  >
                    {item}
                  </div>
                ))}
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {detail.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1 bg-muted border border-border/60 rounded-full text-xs text-muted-foreground"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {images.map((image, i) => (
                    <button
                      key={i}
                      className="group relative aspect-video rounded-xl overflow-hidden bg-muted border border-border/60 hover:border-orange-500/50 transition-all duration-200 text-left focus:outline-none focus:ring-2 focus:ring-orange-500/40"
